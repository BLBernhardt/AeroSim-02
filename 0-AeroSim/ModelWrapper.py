...
from telemetry import TmUDP, State
...
class Buses():
    def __init__(self, toIp="127.0.0.1", toPort=5001):
        self.mousePos     = (0,0)
...
class Model():
    ...
    def physics(self, mode) -> None:
        ...
        ### Update state bus
        ftToM = 1/3.28
        state = self.data.tm.tm
        state.latY_m   =  mdl.position.x*ftToM
        state.lonX_m   =  mdl.position.y*ftToM
        state.hae_m    = -mdl.position.z*ftToM

        state.velNorth_mps =  mdl.Ve.x*ftToM
        state.velEast_mps  =  mdl.Ve.y*ftToM
        state.velUp_mps    = -mdl.Ve.z*ftToM

        state.accForward_mps2 =  mdl.Ab.x*ftToM
        state.accRight_mps2   =  mdl.Ab.y*ftToM
        state.accUp_mps2      = -mdl.Ab.z*ftToM
