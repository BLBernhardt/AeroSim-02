...
//================== Init ====================================================================================================
 	...
 	{
 		first_pass = 0;
 		Aero_D->Units = FPS;    // put this in configue file later
...
//================ Controls ================================================================================================
 	...
 	// CTRL->Elevator_Cmd 		 Elevator deflection 	[- 1, 1 ]. 	 
	// CTRL->Aileron_Cmd 		 Aileron deflection 	[- 1, 1 ]. 	 
    // double Rud_Defl; 		 Eudder deflection 		[- 1, 1 ]. 	 	 
 	// CTRL->Throttle_Cmd 		 Throttle setting 		[ 0,  1 ]. 

//	printf(" >>>> %f %f %f \n", CTRL->Elevator1_Cmd, CTRL->Aileron_Cmd,   CTRL->Rudder_Cmd );


	delta_e_deg  = -(CTRL->Elevator_Cmd * ELV_MAX_ANG ) ; 		// Pitch stick y axis range -1.0 to 1.0 
 	delta_e_trim = ELV_TRIM;
 	delta_e 	 = (delta_e_deg + delta_e_trim) * DEGtoRAD;
 
	delta_a_deg  = (CTRL->Aileron_Cmd * AIL_MAX_ANG ) / 5.0 ; 	// Roll stick x axis range -1.0 to 1.0 
 	delta_a 	 = delta_a_deg * DEGtoRAD;	

	delta_r_deg  = (CTRL->Rudder_Cmd * RUD_MAX_ANG ); 			// Roll stick x axis range -1.0 to 1.0 
 	delta_r 	 = delta_r_deg * DEGtoRAD;	
 
	Thrust = CTRL->Throttle_Cmd * MAX_THRUST; 					// Throttle Command setting [ 0, 1]. 		 

 	Weight = WEIGHT; 
 	 
 //------- Test Code ------------------------------------------------ 
 	#if 0	 	
 	 	printf("4] Thrst %3.2f Elev %3.2f %3.2f Pitch %7.4f \n", 	Thrust, delta_e_deg, CTRL->Elevator_Cmd, Pitch );
 	#endif	 
 	#if 0	 	
...
//===============================================================	
		...
		if( Aero_D->Units == METRIC )	Aero_D->G_meter = Wr/32.2;
		if( Aero_D->Units == FPS ) 		Aero_D->G_meter = Wr/9.81;	
...
//printf(" >>> Roll %f \n", Roll ); 
...
#if 0
	{
 		...
 		time, flight_path_ange, Vinf, altitude, delta_e * RADtoDEG , CM_DELTA_E * delta_e * RADtoDEG, q * RADtoDEG, Pitch * RADtoDEG ); 
 		printf( "-------------------------------------------------------------------------------------------------------------------------------------\n" );
...
// TM UDP Interface
#if 0
		TM->Pitch    = Aero_D->Pitch_d;
		TM->Roll     = Aero_D->Roll_d;
		TM->Yaw      = Aero_D->Yaw_d; 	
		TM->AirSpeed = Aero_D->AirSpeed_knots;
		TM->Alt      = Aero_D->Altitude_AGL_feet;
		TM->Alpha    = Aero_D->Alpha;
		TM->Beta     = Aero_D->Beta;
		TM->Climb    = Aero_D->RateOfClimb_ft_sec;
