...
void PFD_AltTicker( int Pos_X, int Pos_Y,  float ALT )
{
...
float fontHeight = .30;
 	...
 	//	glfwGetWindowSize( window, &width, &height );
 		...
 		{																			// Implemeted better glscissor coordinates for x axis only, need simular for Y 
 			glScissor( 	(float)window_x/2.0 + (float)window_y * (Pos_X  + 160)/1100, // winddow size see glutGet(GLUT_WINDOW_WIDTH ) glutGet(GLUT_WINDOW_HEIGHT )
