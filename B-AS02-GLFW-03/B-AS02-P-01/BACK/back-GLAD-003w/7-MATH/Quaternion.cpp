...
void Quaternion_InitXYZW(Quaternion4* q, double x, double y, double z, double w)
{
    if (q != NULL)
    {
        q->x = x;
...
Quaternion4 multiply_quaternions( Quaternion4 q1, Quaternion4 q2 ) 
{
    Quaternion4 result;
...
Quaternion4 quaternion_normalize( Quaternion4 q  ) 
{
    double n;
    n = sqrt( q.x*q.x + q.y*q.y + q.z*q.z + q.w*q.w );
    return  ( Quaternion4 ){ (float)(q.w/n), (float)(q.x/n), (float)(q.y/n), (float)(q.z/n) };
}
...
void quaternion_to_euler( Quaternion4 q, double *pitch, double *yaw, double *roll ) 
{
...
//=======================================================================================================
Quaternion4 euler_to_quaternion( double pitch, double yaw, double roll ) 
{
...
Quaternion4 make_rotation_quaternion_from_axis_and_angle(  Quaternion4 axis, float angle  ) 
{
   Quaternion4 q;
   q = ( Quaternion4 ){ cos( angle/2.0 ),           axis.x * sin( angle/2.0 ), 
                       axis.y * sin( angle/2.0 ),  axis.z *sin( angle/2.0 )    };
   return q; 
}

Quaternion4 Quat_to_Axis( Quaternion4 q1 ) 
{
   ...
   return ( Quaternion4 ){ a, x2, y2, z2 };
}



//======================================================================================
//  from flightmodel-810-03
