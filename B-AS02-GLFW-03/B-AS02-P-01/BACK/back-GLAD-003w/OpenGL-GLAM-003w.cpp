...
// 	static Aerodyn_t  Aero_Dg = {0};
    ...
    if( interrupt_flag == 1 )  // Timer Interupt set for 1000 interupts per second. See Timer Initialization
    {
    	...
    	{   count_20 = 0;
        	...
        	if (!Update_Graphics( windows )) CTRL->terminate = 1;    // all windows closed → exit
...
//===========================================================================================================================================
    ...
    while( !CTRL.terminate )
    {
 	...
 	/* Graphics Cleanup */
 	if (windows[0])  glfwDestroyWindow(windows[0]);
 	if (windows[1])  glfwDestroyWindow(windows[1]);
 	if (windows[2])  glfwDestroyWindow(windows[2]);
