...
void Render_2D_Win1( Server_Tx_t* TMrx );
void Render_2D_Win2();
void Render_2D_Win3();


void PFD_Exec( int offset_X , int offset_Y, Cntrls_t *Cntrls, Aerodyn_t  *Aero_Dg   );
void  EDL_Map();
 Vector3 getAttitude();
 Vector3 getPosition();
 void drawAirvehicle(void);
...
void Render_2D_Win1( GLFWwindow* windows[3], Server_Tx_t* TMrx ) 
{
...
//	float		winWidth, winHeight;
	...
	static float yaw   = 0.0;
  	...
  	DSPLg.Win1_Width	= 500;
  	DSPLg.Win1_Height	= 500;
  	
  	
	glViewport( 0, 0, DSPLg.Win1_Width, DSPLg.Win1_Height );
  	glMatrixMode( GL_PROJECTION );
  	glLoadIdentity(  );
  	gluPerspective( 60, (float)DSPLg.Win1_Width/(float)DSPLg.Win1_Height, 5, 100 );
...
void Render_2D_Win2( GLFWwindow* windows[3], Cntrls_t* CTRL ) 
{
...
//     winHeight = (float)glutGet( GLUT_WINDOW_HEIGHT );
     ...
     {
         DSPLg.Grid_X = (int)(1100.0 * ( winHeight/winWidth ));
         DSPLg.Grid_Y = 1100;
     }

     if( winHeight <= winWidth ) 
     {
         DSPLg.Grid_Y = (int)(1100.0 * ( winWidth/winHeight ));
         DSPLg.Grid_X = 1100;
     }

     gluOrtho2D( 0,  DSPLg.Grid_Y, 0, DSPLg.Grid_X ); 
     glMatrixMode( GL_MODELVIEW );
     ...
     glPushMatrix();
		...
		PFD_Exec( 0, 0, CTRL, &Aero_Dg );  //  Position X and Y relative to center of window. 
	#if 0
...
void Render_2D_Win3( GLFWwindow* windows[3]  ) 
{
 float		winWidth, winHeight;
 int  DG_X, DG_Y;
// int Pos_X, Pos_Y;
