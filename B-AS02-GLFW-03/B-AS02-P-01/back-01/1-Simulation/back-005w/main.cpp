...
//====== Graphics ==============================
	...
	bool Update_Graphics( GLFWwindow* windows[4], Server_Tx_t* TMrx, Display_t* DSPL, Cntrls_t* CTRL );
	void Render_2D_Win1( GLFWwindow* windows[4], Server_Tx_t* TMrx, Display_t* DSPL );
	void Render_2D_Win2( GLFWwindow* windows[4], Server_Tx_t* TMrx, Display_t* DSPL, Cntrls_t* CTRL ); 
	void Render_2D_Win3( GLFWwindow* windows[4], Server_Tx_t* TMrx, Display_t* DSPL );
...
//======== IPC UDP ===============================
	...
	void UDP_init( );
	void UDP_Rx( Client_Tx_t* TMrx );
	void UDP_Tx( Server_Tx_t* TMtx );

	void TM_load( Server_Tx_t* TM_Stx, Client_Tx_t* TM_Ctx, Aerodyn_t* AERO_D, Cntrls_t* CTRL );
	void TM_Print( KB_State_t *kb, Server_Tx_t* TMtx, Client_Tx_t* TMrx );	
...
void Process_Timing( Cntrls_t* CTRL )
{
 ...
 static int count_10	= 0; 
	...
	static struct Server_Tx_t TM_Stx	= {0};	
...
//-----------------------------------------------------------------------			
		...
		{ count_20 = 0;
			...
			float dt = 0.020; // sec.	
	 		...
	 		if( !Update_Graphics( windows, &TM_Stx, &DSPL, CTRL )) CTRL->terminate = 1; // all windows closed → exit
...
bool Update_Graphics( GLFWwindow* windows[4], Server_Tx_t* TM_Stx, Display_t* DSPL, Cntrls_t* CTRL )
{
...
void UDP_Tx( Server_Tx_t* TMtx )
{
 ...
 size_t pkt_len;
	...
	pkt_len = Prepare_UDP_Msg( TMtx, sizeof( Server_Tx_t ), tx_buf, sizeof( tx_buf ));
	...
	int status = IPC_UDP_S_Tx( tx_sockfd, &tx_addr, tx_buf, pkt_len );
