...
//  SET OPERATION MODE
//=================================================================================
//  GSP:  Guy Stoffer  Python 
//  BBC:  Brian Bernhard C
//  FULL: Both Sim and Display are active
//  S_SIM: Server Simulation and UDP Tx 
//  C_DIS: Client Display   and  UDP Rx 
//

#if 0
enum ( FULL, S_SIM, C_DIS, BBC, GSP );
#define MODE  		FULL;
#define IPC_CONFIG	BBC;

#if IPC_CONFIG == BBC;
	#include "IPC_UDP_Common_BBC.h"	
#endif
#if IPC_CONFIG == GSP;
	#include "IPC_UDP_Common_GSP.h"	
#endif
#endif
...
//====== Graphics ==============================
	...
	bool Update_Graphics( GLFWwindow* windows[4], TM_Param_t* TMrx, Display_t* DSPL, Cntrls_t* CTRL );
	void Render_2D_Win1( GLFWwindow* windows[4], TM_Param_t* TMrx, Display_t* DSPL );
	void Render_2D_Win2( GLFWwindow* windows[4], TM_Param_t* TMrx, Display_t* DSPL, Cntrls_t* CTRL ); 
	void Render_2D_Win3( GLFWwindow* windows[4], TM_Param_t* TMrx, Display_t* DSPL );
	void Render_2D_Win4( GLFWwindow* windows[4], TM_Param_t* TMrx, Display_t* DSPL, Cntrls_t* CTRL ); 	
...
//======== IPC UDP ===============================
	...
	int  IPC_UDP_C_Rx(int sockfd, TM_Param_t* TMrx);
...
float Time_Update( TM_Param_t *TMtx )
{
 struct timespec ts;
...
void Process_Timing( Cntrls_t* CTRL )
{
 ...
 static int count_10	= 0; 
	...
	static struct Client_Tx_t TM_Ctx	= {0}; 
	static struct GSP_UDP_t   TM_Stx	= {0};	
	static struct TM_Param_t  TM_Disp	= {0};		
...
// 50.0 FPS 	
		...
		{ count_20 = 0;
			...
			float dt = 0.020; // sec.	
	 		Aero_Computations( dt, &AERO_D, CTRL );
	 		if( !Update_Graphics( windows, &TM_Disp, &DSPL, CTRL )) CTRL->terminate = 1; // all windows closed → exit
...
bool Update_Graphics( GLFWwindow* windows[4], TM_Param_t* TM_Stx, Display_t* DSPL, Cntrls_t* CTRL )
{
...
( windows[3] && glfwWindowShouldClose( windows[3] )) )
 {
 ...
 if( windows[3] ) glfwSetWindowShouldClose( windows[3], GLFW_TRUE );
 return false;
 }

 Render_2D_Win1( windows, TM_Stx, DSPL );
 Render_2D_Win2( windows, TM_Stx, DSPL, CTRL );
 Render_2D_Win3( windows, TM_Stx, DSPL );
 Render_2D_Win4( windows, TM_Stx, DSPL, CTRL );
...
//	pkt_len = Prepare_UDP_Msg( TMtx, sizeof( TM_Param_t ), tx_buf, sizeof( tx_buf ));
#if 0
	if( pkt_len == 0 ) 
	{
