...
bool Update_Graphics(GLFWwindow* windows[3], Cntrls_t* CTRL, Aerodyn_t* AERO_D,  Display_t* DSPL, Server_Tx_t*  TMrx  );
void Render_2D_Win1( GLFWwindow* windows[3], Server_Tx_t* TMrx, Display_t* DSPL );
void Render_2D_Win2( GLFWwindow* windows[3], Cntrls_t* CTRL, Aerodyn_t* AERO_D, Display_t*	DSPL   );
void Render_2D_Win3( GLFWwindow* windows[3],  Aerodyn_t* AERO_D, Display_t*	DSPL   );
...
bool Update_Graphics(GLFWwindow* windows[3], Cntrls_t* CTRL, Aerodyn_t* AERO_D,  Display_t* DSPL, Server_Tx_t*  TMrx  )
{
         ...
         (windows[2] && glfwWindowShouldClose(windows[2])) )
    {
     ...
     Render_2D_Win1( windows, TMrx, DSPL );
     Render_2D_Win2( windows, CTRL, AERO_D, DSPL  );
     Render_2D_Win3( windows, AERO_D, DSPL );
...
void Process_Timing( Cntrls_t* CTRL )
{
    ...
    static int count_10 	= 0;  
 	...
 	struct Aerodyn_t 	AERO_D 	= {0}; 
 	struct Display_t 	DSPL 	= {0}; 
	struct Client_Tx_t  TMtx 	= {0}; 
	struct Server_Tx_t  TMrx 	= {0};  	
    ...
    if( interrupt_flag == 1 )  // Timer Interupt set for 1000 interupts per second. See Timer Initialization
    {
    	...
    	{   count_10 = 0;   	
			CTRL->Elevator_Cmd = TMtx.pitch_cmd;
			CTRL->Aileron_Cmd  = TMtx.roll_cmd;
			CTRL->Rudder_Cmd   = TMtx.yaw_cmd;
			CTRL->Throttle_Cmd = TMtx.thrtl_cmd;
		}		
    	...
    	{   count_20 = 0;
        	...
        	if (!Update_Graphics( windows, CTRL, &AERO_D, &DSPL, &TMrx )) CTRL->terminate = 1;    // all windows closed → exit
...
//    	    UDP_Tx( &TMtxg );
    	...
    	{   count_100 = 0;
			Time_Update( &TMrx );
			IPC_UDP_C_Tx(tx_sock, &dest_addr, &TMtx ); 			
			TM_Print( &kb, &TMrx, &TMtx );
		}	
	}
}
...
printf("         Needed to reset terminal to normal mode after progrm ends \n\n");
  ...
  //      if (!Update_Graphics( windows )) break;   // all windows closed → exit
    	Process_Timing( &CTRL );
