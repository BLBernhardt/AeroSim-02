...
//void Render_Window2(GLFWwindow* windows[3],  Cntrls_t* CTRL );
...
bool Update_Graphics(GLFWwindow* windows[3], Cntrls_t* CTRL )
{
    ...
    // Render each window
     Render_2D_Win1( windows, &TMrxg );
//    Render_Window1( windows );
     Render_2D_Win2( windows, CTRL );
     Render_2D_Win3( windows );
...
// 	static Aerodyn_t  Aero_Dg = {0};
    ...
    if( interrupt_flag == 1 )  // Timer Interupt set for 1000 interupts per second. See Timer Initialization
    {
    	...
    	{   count_20 = 0;
        	...
        	if (!Update_Graphics( windows, CTRL )) CTRL->terminate = 1;    // all windows closed → exit
