...
void  stroke_output(GLfloat, GLfloat, GLfloat, char* );





void Flight_Stick( int Pos_X, int Pos_Y )
{
...
static int timer;
 	...
 	{
 		Cntrls.Stick_X = (float)Cntrls.MouseX/0.8 ;
  		Cntrls.Stick_Y = (float)Cntrls.MouseY/0.8  ;
  		if( Cntrls.Stick_X > +100.0 ) Cntrls.Stick_X = +100.0 ;
  		if( Cntrls.Stick_X < -100.0 ) Cntrls.Stick_X = -100.0 ;  	
   		if( Cntrls.Stick_Y > +100.0 ) Cntrls.Stick_Y = +100.0 ; 	
   		if( Cntrls.Stick_Y < -100.0 ) Cntrls.Stick_Y = -100.0 ; 	
 	}
 	if( !Cntrls.Stick_Enable )
 	{ 
 		if( Cntrls.Stick_X > 0 ) Cntrls.Stick_X = Cntrls.Stick_X - 0.1;
 		if( Cntrls.Stick_X < 0 ) Cntrls.Stick_X = Cntrls.Stick_X + 0.1;
 		if( Cntrls.Stick_Y > 0 ) Cntrls.Stick_Y = Cntrls.Stick_Y - 0.1;
 		if( Cntrls.Stick_Y < 0 ) Cntrls.Stick_Y = Cntrls.Stick_Y + 0.1; 	
 	}
	...
	y_pos = (float)(-Cntrls.Stick_Y ); 	
  	...
  	Cntrls.y_cmd = y_pos/100.0;   //   range -1.0 to + 1.0
