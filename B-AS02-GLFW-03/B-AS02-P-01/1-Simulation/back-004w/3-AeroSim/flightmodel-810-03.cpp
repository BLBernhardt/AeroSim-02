...
extern AeroModel* Aero_M;     // ← Must be here
void Aero_Computations( float dt, const AeroModel* Aero_M,  Server_Tx_t* TM );
...
//================ Controls ================================================================================================
 	...
 	// CTRLg.Elevator_Cmd 		 Elevator deflection 	[- 1, 1 ]. 	 
	// CTRLg.Aileron_Cmd 		 Aileron deflection 	[- 1, 1 ]. 	 
    // double Rud_Defl; 		 Eudder deflection 		[- 1, 1 ]. 	 	 
 	// CTRLg.Throttle_Cmd 		 Throttle setting 		[ 0,  1 ]. 

	delta_e_deg  = -(CTRLg.Elevator_Cmd * ELV_MAX_ANG ) ; 		// Pitch stick y axis range -1.0 to 1.0 
 	delta_e_trim = ELV_TRIM;
 	delta_e 	 = (delta_e_deg + delta_e_trim) * DEGtoRAD;
 
	delta_a_deg  = (CTRLg.Aileron_Cmd * AIL_MAX_ANG ) / 5.0 ; 	// Roll stick x axis range -1.0 to 1.0 
 	delta_a 	 = delta_a_deg * DEGtoRAD;	

	delta_r_deg  = (CTRLg.Rudder_Cmd * RUD_MAX_ANG ); 			// Roll stick x axis range -1.0 to 1.0 
 	delta_r 	 = delta_r_deg * DEGtoRAD;	
 
	Thrust = CTRLg.Throttle_Cmd * MAX_THRUST; 					// Throttle Command setting [ 0, 1]. 		 

 	Weight = WEIGHT; 
 	 
 //------- Test Code ------------------------------------------------ 
 	#if 0	 	
 	 	printf("4] Thrst %3.2f Elev %3.2f %3.2f Pitch %7.4f \n", 	Thrust, delta_e_deg, CTRLg.Elevator_Cmd, Pitch );
 	#endif	 
 	#if 0	 	
...
// Update the airspeed 
	...
	Aero_Dg.Alpha = Alpha * RADtoDEG; 
	Aero_Dg.Beta  = Beta * RADtoDEG;
...
#if 0   
    if (Aero_M && Aero_M->tables_loaded) 
    {
        CL2 = CoeffTable1D_Interpolate(&Aero_M->CL_alpha, Alpha * RADtoDEG);
    } 
    else 
    {
        printf("ERROR >>> Tables not loaded\n");
    }
#endif
    
   // printf(">>FM-1 Alpha %f  CL2 %f \n", Alpha_tst, CL2  );		
...
//============================================================================================================================
	 		...
	 		Aero_Dg.Position_inertial_X_ft = position.x;
			Aero_Dg.Position_inertial_Y_ft = position.y;
			Aero_Dg.Position_inertial_Z_ft = position.z;
...
// PFD Interface 
 		...
 		Aero_Dg.Pitch_d = Pitch * RADtoDEG;
 		Aero_Dg.Roll_d 	= Roll 	* RADtoDEG;
 		Aero_Dg.Yaw_d 	= Yaw 	* RADtoDEG; 
	
		if( strcmp( UNITS, "Imperial" ) == 0 )
		{	
			Altitude_ft += Vw * dt;
 			Aero_Dg.RateOfClimb_ft_sec = Vw * 60.0 ;
	
			if( Altitude_ft > 50000.0 ) 	Altitude_ft = 50000.0 ;
			if( Altitude_ft < 0.0 ) 		Altitude_ft = 0.0 ;		 
	 
			Aero_Dg.Altitude_AGL_feet = Altitude_ft; 
			Aero_Dg.AirSpeed_knots = Vu * 0.592; // ft/sec = 0.592 knots. 
		}
...
// TM UDP Interface
		...
		TM->Pitch    = Aero_Dg.Pitch_d;
		TM->Roll     = Aero_Dg.Roll_d;
		TM->Yaw      = Aero_Dg.Yaw_d; 	
		TM->AirSpeed = Aero_Dg.AirSpeed_knots;
		TM->Alt      = Aero_Dg.Altitude_AGL_feet;
		TM->Alpha    = Aero_Dg.Alpha;
		TM->Beta     = Aero_Dg.Beta;
		TM->Climb    = Aero_Dg.RateOfClimb_ft_sec;
