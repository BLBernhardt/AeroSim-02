...
//====== Aero Simulation =============================
	...
	void Aero_Computations(float dt, Aerodyn_t* Aero_D, Cntrls_t* CTRL, GD_OUT_t* GD_out );
...
//	void Render_2D_Win4( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL ); 	
	...
	void Intf_ASD_GD( GD_IN_t* GD_in,  GD_OUT_t* GD_out, Aerodyn_t* AERO_D, Cntrls_t* CTRL  );
	void Panel( GD_IN_t* TM_Tx, Display_t* DSPL, Cntrls_t* CTRL );
 	bool Update_Graphics( GLFWwindow* windows[4], GD_IN_t* GD_in, GD_OUT_t* GD_out, Display_t* DSPL, Cntrls_t* CTRL );
...
//======== IPC UDP ===============================
	...
	void UDP_Rx( GSP_UDP_t*  GD_in );

	int  IPC_UDP_C_Rx(int sockfd, GD_IN_t* GD_in);
	void Intf_BBC_GSP( GD_IN_t* TM_Disp, GSP_UDP_t*  GSP, Cntrls_t* CTRL  );
	void Intf_ASD_GD( GD_IN_t* G_Displ,  Aerodyn_t* AERO_D, Cntrls_t* CTRL  );
	void TM_Print( GD_IN_t* GD, TM_Ctx_t* GD_in );	
...
//	static KB_State_t kb				= {0}; 
	...
	static struct GD_OUT_t  	GD_out	= {0};			
...
// 50.0 FPS 	
		...
		{ count_20 = 0;
			...
			float dt = 0.020; // sec.	
	 		Aero_Computations( dt, &ASD, CTRL, &GD_out  );
	 		if( !Update_Graphics( windows, &GD_in, &GD_out, &DSPL, CTRL )) CTRL->terminate = 1; // all windows closed → exit
...
// 10.0 FPS 
		...
		{ count_100 = 0;
			...
			Intf_ASD_GD( &GD_in, &GD_out, &ASD, CTRL  );
