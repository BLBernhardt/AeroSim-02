...
#define PITCH_RATE_MAX	25.0 * DEGtoRAD;	
#define YAW_RATE_MAX	20.0 * DEGtoRAD;
#define ROLL_RATE_MAX	50.0 * DEGtoRAD;
...
void Aero_Computations(float dt, Aerodyn_t* Aero_D, Aero_Cntl_t* AERO_CTRL, Cntrls_t* CTRL )
{
	...
	if( first_pass )
	{
		...
		_q = quat_identity();
		AERO_CTRL->Thr_Cmd_u = THRTL_INIT;
	}
	...
	// Controls
	//------------------------------------------------------------------
	// printf(" %f %f %f \n",  AERO_CTRL->Elv_Cmd_u, AERO_CTRL->Ail_Cmd_u, AERO_CTRL->Rud_Cmd_u ); 
	
	double 	delta_Elv_deg = ( AERO_CTRL->Elv_Cmd_u * ELV_MAX_ANG ) + ( -AERO_CTRL->Elv_Trim_Cmd_u * ELV_MAX_ANG * 0.3 );
	double 	delta_Elv     = delta_Elv_deg * DEGtoRAD  * 0.3;  //  Need to investigate this - should not need this 0.3

	double 	delta_Ail_deg = ( AERO_CTRL->Ail_Cmd_u * AIL_MAX_ANG) / 5.0;
	double 	delta_Ail     = delta_Ail_deg * DEGtoRAD;

	double 	delta_Rud_deg = ( AERO_CTRL->Rud_Cmd_u * RUD_MAX_ANG);
	double 	delta_Rud     = delta_Rud_deg * DEGtoRAD;

	double 	Thrust = AERO_CTRL->Thr_Cmd_u * MAX_THRUST;
	...
	double Mm = qSc * (CM_0
					 + CM_ALPHA   * Alpha
					 + CM_Q       * (Wq * _C / (2.0 * Vinf))
					 + CM_DELTA_E * delta_Elv);
#else
	// original raw-rate form (active while FIX_CMQ_NONDIM == 0)
	double Mm = qSc * (CM_DELTA_E * delta_Elv
					 + CM_ALPHA   * Alpha
					 + CM_Q       * Wq);
#endif
	double Aq = Mm / _Iyy;
...
#define MAX_RATE_PITCH	25.0 * DEGtoRAD
#define MAX_RATE_YAW	20.0 * DEGtoRAD
#define MAX_RATE_ROLL	50.0 * DEGtoRAD

	if( Wq > +MAX_RATE_PITCH ) Wq = +MAX_RATE_PITCH;
	if( Wq < -MAX_RATE_PITCH ) Wq = -MAX_RATE_PITCH;
	if( Wr > +MAX_RATE_YAW   ) Wr = +MAX_RATE_YAW;
	if( Wr < -MAX_RATE_YAW   ) Wr = -MAX_RATE_YAW;
	if( Wp > +MAX_RATE_ROLL  ) Wp = +MAX_RATE_ROLL;
	if( Wp < -MAX_RATE_ROLL  ) Wp = -MAX_RATE_ROLL;
