...
void Render_2D_Win1( GLFWwindow* windows[3], GD_IN_t* TMrx, Display_t* DSPL );
void Render_2D_Win2( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL );
void Render_2D_Win3( GLFWwindow* windows[3], GD_IN_t* GD_in, Display_t* DSPL );
void Render_2D_Win4( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL );

void PFD_Exec( int offset_X , int offset_Y, GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t *CTRL );
void HSI_Map( GD_IN_t* TM );
...
bool Update_Graphics( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL )
{
 // Check if any window was closed
 	...
 	Render_2D_Win1( windows, GD_in, DSPL );
 	Render_2D_Win2( windows, GD_in, DSPL, CTRL );
 	Render_2D_Win3( windows, GD_in, DSPL );
 	Render_2D_Win4( windows, GD_in, DSPL, CTRL );
...
void Render_2D_Win2( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL ) 
{
 ...
 if (!windows[1] || glfwWindowShouldClose(windows[1])) return;
	...
	glPushMatrix();
		...
		PFD_Exec( 0, 0, GD_in, DSPL, CTRL ); // 0, 0,  Position X and Y relative to center of window. 
#if 0
...
void Render_2D_Win4( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL ) 
{
 ...
 if (!windows[3] || glfwWindowShouldClose(windows[3])) return;

	glfwMakeContextCurrent(windows[3]);
	glPushAttrib(GL_ALL_ATTRIB_BITS);
	glDisable( GL_DEPTH_TEST );
	glDisable( GL_LIGHTING );
	glDisable( GL_TEXTURE_2D );
