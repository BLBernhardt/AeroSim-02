...
//====== Aero Simulation =============================
	void Aero_Computations(float dt, Aerodyn_t* Aero_D, Aero_Cntl_t* AERO_CTRL, Cntrls_t* CTRL );
...
//====== Graphics ==============================
	...
	bool Update_Graphics( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL );

//	void Render_2D_Win1( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL );
//	void Render_2D_Win2( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL ); 
//	void Render_2D_Win3( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL );
//	void Render_2D_Win4( GLFWwindow* windows[4], GD_IN_t* GD_in, Display_t* DSPL, Cntrls_t* CTRL ); 	

 	GLFWwindow* windows[4] = {NULL, NULL, NULL};
	void TM_Interface( TM_Stx_t* TM_in, TM_Ctx_t* TM_out, GD_IN_t* GD_in, Aerodyn_t* Aero_D, Aero_Cntl_t* AERO_CTRL, Cntrls_t* CTRL );
//	void Intf_ASD_GD( GD_IN_t* GD_in,  Aerodyn_t* AERO_D, Cntrls_t* CTRL  );
	void Panel(  Display_t* DSPL, Cntrls_t* CTRL );
 	bool Update_Graphics( GLFWwindow* windows[4], GD_IN_t* GD_in,  Display_t* DSPL, Cntrls_t* CTRL );
...
//======== IPC UDP ===============================
	...
	void UDP_init( );
	void UDP_Tx( TM_Ctx_t* TM_out );
	void UDP_Rx( TM_Stx_t* TM_in  );

	int  IPC_UDP_C_Rx(int sockfd, GD_IN_t* GD_in );

void TM_interface( TM_Stx_t* TM_in, TM_Ctx_t* TM_out, GD_IN_t* GD_in, Cntrls_t* CTRL );
...
float Time_Update( GD_IN_t *TMtx )
{
 struct timespec ts;
...
void Process_Timing( Cntrls_t* CTRL )
{
 ...
 static int count_10	= 0; 
	...
	static struct Aerodyn_t		AERO_D		= {0}; 
	static struct Display_t		DSPL		= {0}; 
	static struct TM_Ctx_t 		TM_out		= {0}; 
	static struct TM_Stx_t	 	TM_in		= {0};	
	static struct GD_IN_t  		GD_in		= {0};
	static struct Aero_Cntl_t 	AERO_CTRL	= {0};
...
// 100 FPS		
		...
		{ count_10 = 0;	
			...
			glfwPollEvents();				 // Polls callback functions,  Mouse, Keyboard
		}		
//-----------------------------------------------------------------------	
// 50 FPS 	
		if( count_20 > 20 ) 
		{ count_20 = 0;
			...
			TM_Interface( &TM_in, &TM_out, &GD_in, &AERO_D, &AERO_CTRL, CTRL  );
			Aero_Computations( dt, &AERO_D, &AERO_CTRL, CTRL );
	 		if( !Update_Graphics( windows, &GD_in, &DSPL, CTRL )) CTRL->terminate = 1; // all windows closed → exit

	 		CTRL->frame_cnt++;
			//	TM_load( &TM_in, &TM_out, &AERO_D, CTRL );
			//	UDP_Tx( &TM_out );
...
// 10 FPS 
		...
		{ count_100 = 0;
			...
			//Intf_BBC_GSP( &TM_Disp, &TM_in, CTRL );
...
//	pkt_len = Prepare_UDP_Msg( TMtx, sizeof( GD_IN_t ), tx_buf, sizeof( tx_buf ));
#if 0
	if( pkt_len == 0 ) 
	{
...
//=====================================================================	
   ...
   // glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_COMPAT_PROFILE);	//  this works with OpenGL 3.3 but not OpenGL 3.0
	...
	for( int i = 0; i <= 3; ++i ) 
	{
		...
		glfwSetWindowUserPointer( windows[i], &CTRL );	// Attached  struct pointer to window
	...
	glfwSetWindowUserPointer(windows[1], &CTRL);   // store pointer
...
//=========================================================================================================================
	...
	for (int i = 0; i < 4; i++) if( windows[i] ) glfwDestroyWindow( windows[i] );
