...
void PFD_Exec( int offset_X , int offset_Y  )
{
 int  DG_X, DG_Y;
 int Pos_X, Pos_Y;
 //float scale;
 ...
 //printf("24 %d %d \n", offset_X, offset_Y );
		...
		glPushMatrix();
			...
			Pos_X = 520 - X;  Pos_Y = 0 ;
			PFD_RateOfClimb(  Pos_X, Pos_Y,   Aero.RateOfClimb_ft_sec  );  			


			Pos_X = 275 - X;   Pos_Y = 0 ;   // -56
			PFD_AltTicker(   Pos_X, Pos_Y, DG_X, DG_Y, offset_X, offset_Y, Aero.Altitude_AGL_feet  );  
			
			Pos_X = -390 - X;  Pos_Y = -350 ;
			PFD_SpeedTape(  Pos_X, Pos_Y,  Aero.AirSpeed_knots ); 
			
			Pos_X = -390 - X;  Pos_Y = -48 ;
			PFD_SpeedTicker(  Pos_X, Pos_Y, DG_X, DG_Y, offset_X, offset_Y,  Aero.AirSpeed_knots );

			Pos_X = 0 - X;  Pos_Y = -520 ;
			PFD_Heading( Pos_X, Pos_Y, Aero.Yaw_d  );
			
			Pos_X = 340 - X;  Pos_Y = -580 ;
			Flight_Stick( Pos_X, Pos_Y );
